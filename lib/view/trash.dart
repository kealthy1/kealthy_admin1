...
class UpdateProductTypePage extends StatelessWidget {
  const UpdateProductTypePage({Key? key}) : super(key: key);

  Future<void> updateAllProductsTypeToGrocery(BuildContext context) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('✅ All products updated with Type: Grocery'),
        ),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(
        child: ElevatedButton(
          onPressed: () => updateAllProductsTypeToGrocery(context),
          child: const Text('Update All Products to Type: Grocery'),
        ),
      ),
    );
  }
